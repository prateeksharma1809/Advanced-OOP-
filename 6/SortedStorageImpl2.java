 ...
 *
 */
public class SortedStorageImpl2 implements SortedStorage2 {

	private Node2 head = null;
	private int nullCount = 0;
	private boolean allowDuplication;
	
	public SortedStorageImpl2(boolean allowDuplication) {
	...
	public boolean add(Comparable data) {
		...
		if (head == null || 0 < head.getData().compareTo(data)) {
			...
			if ((current.getNext() == null && !current.getData().equals(data)) || (!find(data) || allowDuplication)) {
				current.setNext(new Node2(data, current.getNext()));
				return true;
			}
		}
		return false;
